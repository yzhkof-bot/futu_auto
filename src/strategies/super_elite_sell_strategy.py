...
class SuperEliteSellStrategy:
    ...
    def calculate_factors(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['F12_CCI_overbought'] = df[cci_col] > 100
    ...
    def identify_sell_points(self, df: pd.DataFrame) -> List[Dict]:
        ...
        # 超级精英卖点：15个因子全部满足
        df['super_elite_sell_signal'] = df['factor_count'] == 15
        
        # 提取卖点信息
        sell_points = []
        for idx, row in df[df['super_elite_sell_signal']].iterrows():
            factor_details = {}
...
def find_super_elite_sell_points(symbol: str, 
                                start_date: str = None, end_date: str = None,
                                cooldown_days: int = 10,  # 默认10天冷却期
    ...
    try:
        ...
        if print_result:
            ...
            print(f"📊 超级精英卖点数: {len(sell_points)}")
            ...
            if sell_points:
                ...
                print("🎯 未找到满足15个因子全部条件的超级精英卖点")
                print("💡 建议：可以尝试降低因子要求数量")
        
        # 绘制图表
        if plot:
            plot_super_elite_signals(symbol, df, sell_points, save_path)
...
def plot_super_elite_signals(symbol: str, df: pd.DataFrame, sell_points: list, save_path: str = None):
    """绘制超级精英卖点图表"""
    ...
    ax1.plot(df.index, df['SMA_5'], label='SMA 5', alpha=0.6, color='orange')
    ax1.plot(df.index, df['SMA_10'], label='SMA 10', alpha=0.6, color='blue')
    ax1.plot(df.index, df['SMA_20'], label='SMA 20', alpha=0.6, color='green')
    ...
    if sell_points:
        ...
        ax1.scatter(sell_dates, sell_prices, color='red', s=100, 
                   label=f'Super Elite Sell Points ({len(sell_points)})', 
                   zorder=5, marker='v', edgecolors='black', linewidth=2)
    
    ax1.set_title(f'{symbol} - Super Elite Sell Strategy (15 Factors All Required)')
    ...
    ax2.axhline(y=15, color='red', linestyle='--', linewidth=2, label='Super Elite Threshold (15)')
    ax2.axhline(y=10, color='orange', linestyle=':', alpha=0.7, label='Elite Threshold (10)')
    ax2.fill_between(df.index, df['factor_count'], 15, 
                    where=(df['factor_count'] >= 15), 
                    color='red', alpha=0.3, label='Super Elite Zone')
    ...
    # Aroon Up
    aroon_up_col = [c for c in df.columns if 'AROONU' in c][0]
    ax3_twin.plot(df.index, df[aroon_up_col], label='Aroon Up', color='green', alpha=0.7)
    ax3_twin.axhline(y=90, color='green', linestyle='--', alpha=0.5)
    
    ax3.set_ylabel('RSI')
    ax3_twin.set_ylabel('Aroon Up')
    ax3.set_xlabel('Date')
...
if __name__ == '__main__':
    # 示例：查找 AAPL 超级精英卖点并绘图
    sell_points = find_super_elite_sell_points('AAPL', cooldown_days=10, plot=True, 
                                              save_path='reports/aapl_super_elite_sell_points.png')