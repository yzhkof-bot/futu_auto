...
def main():
    ...
    data = yf.download(ticker, start='2022-01-01', end='2024-12-31', progress=False)
    ...
    if trades:
        ...
        for i, trade in enumerate(trades, 1):
            ...
            print(f"{i}. {trade.entry_date.strftime('%Y-%m-%d')} -> {trade.exit_date.strftime('%Y-%m-%d')}: "
                  f"${trade.entry_price:.2f} -> ${trade.exit_price:.2f} ({trade.pnl_pct*100:+.2f}%) {result}")
    ...
    fig.suptitle(f'{ticker} 联合健康 - 多信号策略回测 (2022-2024)', fontsize=14, fontweight='bold')
    ...
    ax3.axhline(y=80, color='red', linestyle='--', linewidth=0.8, alpha=0.7)
    ax3.axhline(y=20, color='green', linestyle='--', linewidth=0.8, alpha=0.7)
    ax3.fill_between(df.index, 20, df['K'], where=df['K'] < 20, alpha=0.3, color='green')
    
    # 标注买点时的K值
    ax3.scatter(buy_signals.index, buy_signals['K'], 
                marker='^', color='green', s=80, zorder=5)
    ...
    ax4.bar(df.index, df['MACD_Hist'], label='Histogram', 
            color=['green' if x >= 0 else 'red' for x in df['MACD_Hist']], alpha=0.5)
    ax4.axhline(y=0, color='black', linestyle='-', linewidth=0.5)
    
    # 标注买点时的MACD
    ax4.scatter(buy_signals.index, buy_signals['MACD'], 
                marker='^', color='green', s=80, zorder=5)
    ...
    # 保存图表
    output_path = 'reports/UNH_strategy_backtest.png'
